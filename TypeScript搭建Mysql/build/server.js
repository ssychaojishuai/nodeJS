...
//保存demo
let sql = "insert into demo(name,age) values (?,?)";
let user = ["tony", "23"];
database_1.query(sql, user, (rdata) => {
    console.log("保存成功");
});
/**
...
database_1.query(getSql, (rdata) => {
    //console.log(rdata[0].name);//可通过[下标].字段获取相应信息
    let dataString = JSON.stringify(rdata);
    ...
    var rrdata = JSON.parse(dataString);
